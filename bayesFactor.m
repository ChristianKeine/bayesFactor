classdef bayesFactor < handle
    ...
    methods (Static, Hidden=false)
        ...
        function [bf10,pValue,stats] = Wilcox(x,y,varargin)
           ...
           % P.addOptional('onesided',false,@(x) islogical(x))
    ...
    methods (Static, Access = private)
        ...
        function bf01 = computeBayesFactorWilcoxon(deltaSamples,scale,onesided)
            
            % get posterior density
            densZeroPoint = ksdensity(deltaSamples(:),0); % logspline used in R/JASP
            
            % For testing
            a = 0;
            priorDenseZeroPoint = scale./(pi*(scale.^2 + (0-a).^2));
            
            
%             if onesided
%                 bf01 =  (priorDensZeroPoint/corFactorPrior)./(densZeroPoint/corFactorPosterior);
%             else
                bf01 = priorDenseZeroPoint./densZeroPoint;
%             end
        end
...
%             yRanks = allRanks((n1+1):(n1+n2));
            ...
            for j=1:nSamples
                for i=randperm(n1+n2)
                    ...
                    allVals(i) = bayesFactor.truncNormSample(under,upper,oldDeltaProp,1);
                end
                ...
                [delta,mu] = bayesFactor.sampleGibbsTwoSample(xVals,yVals,nGibbsIter,cauchyPriorScale);
        ...
        %%
        function [delta, mu] = sampleGibbsTwoSample(x,y,nIter,scale)
        ...
        %%
        function mySample = truncNormSample(lBound,uBound,mu,sd)
